...
-- Commerce money is stored in integer cents and percentage shares in basis
-- points. API representations already use these units; keeping the database in
-- the same units removes floating-point rounding from quote and settlement
...
INSERT INTO listings (
    ...
    listing_mode, status, price_cents, regional_pricing_policy_json,
    vinyl_release_provider, vinyl_release_url, created_by_user_id, created_at, updated_at
)
SELECT
    listing_id, community_id, asset_id, live_room_id, replay_asset_id,
    listing_mode, status, CAST(ROUND(price_usd * 100) AS INTEGER), regional_pricing_policy_json,
    vinyl_release_provider, vinyl_release_url, created_by_user_id, created_at, updated_at
FROM listings_old;
...
CREATE INDEX idx_listings_asset
    ON listings(asset_id) WHERE asset_id IS NOT NULL;
...
INSERT INTO purchase_quotes (
    ...
    asset_id, live_room_id, replay_asset_id, base_price_cents, pricing_tier, final_price_cents,
    ...
    expires_at, consumed_at, failed_at, created_at, updated_at, funding_locked_at
)
SELECT
    quote_id, community_id, listing_id, buyer_kind, buyer_user_id,
    buyer_wallet_address, buyer_wallet_address_normalized, buyer_chain_ref,
    asset_id, live_room_id, replay_asset_id,
    CAST(ROUND(base_price_usd * 100) AS INTEGER), pricing_tier,
    CAST(ROUND(final_price_usd * 100) AS INTEGER),
    CASE
        WHEN allocation_snapshot_json IS NULL THEN NULL
        ELSE (
                ...
                json(
                    json_remove(
                        json_set(
                            allocation.value,
                            '$.amount_cents',
                            CAST(ROUND(COALESCE(json_extract(allocation.value, '$.amount_usd'), 0) * 100) AS INTEGER)
                        ),
                        '$.amount_usd'
                    )
                )
            )
            FROM json_each(purchase_quotes_old.allocation_snapshot_json) AS allocation
        )
    END,
    funding_mode, funding_asset_json, source_chain_json,
    route_provider, funding_destination_address, route_policy_compliant, route_live_available,
    policy_origin, destination_settlement_chain_json, destination_settlement_token,
    destination_settlement_amount_atomic, destination_settlement_decimals, treasury_denomination,
    quote_ttl_seconds, route_required, route_status_policy, route_hop_tolerance,
...
INSERT INTO purchases (
    ...
    buyer_chain_ref, settlement_wallet_attachment_id, purchase_price_cents, pricing_tier,
    settlement_chain, settlement_token, settlement_tx_ref, donation_partner_id,
    donation_share_bps, donation_amount_cents, donation_settlement_ref,
    vinyl_release_provider, vinyl_release_url, created_at, settlement_mode
)
SELECT
    purchase_id, community_id, listing_id, asset_id, live_room_id, replay_asset_id,
    buyer_kind, buyer_user_id, buyer_wallet_address, buyer_wallet_address_normalized,
    buyer_chain_ref, settlement_wallet_attachment_id,
    CAST(ROUND(purchase_price_usd * 100) AS INTEGER), pricing_tier,
    settlement_chain, settlement_token, settlement_tx_ref, donation_partner_id,
    CASE
        WHEN donation_share_pct IS NULL THEN NULL
        ELSE CAST(ROUND(donation_share_pct * 100) AS INTEGER)
    END,
    CASE
        WHEN donation_amount_usd IS NULL THEN NULL
        ELSE CAST(ROUND(donation_amount_usd * 100) AS INTEGER)
    END,
    donation_settlement_ref, vinyl_release_provider, vinyl_release_url, created_at, settlement_mode
FROM purchases_old;
...
SELECT
    purchase_allocation_leg_id, purchase_id, quote_id, community_id, recipient_type, recipient_ref,
    waterfall_position, share_bps, CAST(ROUND(amount_usd * 100) AS INTEGER),
    settlement_strategy, status, settlement_ref, failure_reason, created_at, updated_at,
