-- Repair the divergent 1123 migration lineage. Some live shards applied
-- 1123_song_engagement_activity_timezone.sql but never applied the separately
-- introduced 1123_karaoke_attempts.sql. Keep this replay safe for shards that
-- already have the complete karaoke schema.
CREATE TABLE IF NOT EXISTS karaoke_attempt (
...
CREATE INDEX IF NOT EXISTS idx_karaoke_attempt_rank
    ON karaoke_attempt(
        post_id,
        karaoke_revision_id,
        scoring_version,
...
CREATE INDEX IF NOT EXISTS idx_karaoke_attempt_user_post
