...
- relationship to follows, messaging, and club interactions

It does not cover:

- club-level bans (see [club.md](./club.md))
...
Block should prevent direct social interaction and personal-surface visibility between two users. It does not override shared club visibility. Mute should suppress the muted user's content from the muter's personal feeds without preventing interaction.
...
- the authoritative shared-club exception is defined below under Block And Clubs

### Block And Clubs

Blocks do not override club membership or club visibility.

Rules:

- if both users are members of the same club, they may still see each other's posts within that club's shared feed and thread surfaces
- blocks do not remove club membership
- blocks do not prevent a blocked user from viewing public club content authored by the blocker
- blocks apply to personal surfaces such as `Home`, `Your Clubs`, profile pages, DMs, and user-targeted search — not to shared club feeds or thread reads inside a club

This preserves the club as a shared community space while protecting personal boundaries.
...
- a user who has been blocked may still see the blocker's anonymous posts within a shared club, because the anonymous label does not reveal identity
...
- user A may still see user B's content in club feeds and search
