...
- Should gated access mode gain a subfield like `gate_mode` or `audience_gate_ref` to encode segment types (purchase_entitlement, study_streak, wallet_allowlist), or should v0 rely on club-level gate evaluation and defer room-specific audience segmentation? — RESOLVED for v1.5: see [livestream-audience-gates.md](./livestream-audience-gates.md). Inline `live_rooms.audience_gate_json` with `community_members` + `purchase_entitlement` segments; membership is no longer a universal precondition (a gate pass grants access regardless of membership). `study_streak`/`wallet_allowlist` remain deferred.
