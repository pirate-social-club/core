...
- [ ] A2c. **Received-mail archive coverage survey:** run `survey-mail-archive.mjs` over a recent, local MBOX or Proton EML export from an opt-in mailbox. Stream headers only and make no network requests. Require an explicit trusted receiving `authserv-id`, use only its contemporaneous `Authentication-Results` DKIM pass plus `header.i`/`header.d` identity for alignment coverage, and never treat this observational evidence as gate authorization. Report bulk-inclusive, loose header-filtered, and—when source metadata permits—higher-precision replied-to populations. Collapse repeated correspondents to one best observed verdict per From domain and commit counts only. Record the date window, folder/filter counts, missing trusted verdicts, and correspondent/sector/receiver/interaction bias. Split non-exact passing signatures into signer-subdomain-of-From, From-subdomain-of-signer, provider fallback, and unrelated categories for prevalence only; child-zone control does not authorize parent-domain mailboxes, so these categories do not relax the exact-alignment gate. Validate the method separately against a small explicitly consented recent cryptographic subsample; do not perform DNS during the archive run. This estimates compatibility for observed correspondence, not a population census or organization-wide claim. The provisional Gmail and Proton runs are recorded in `spikes/email-domain-gate/findings.md`; the replied-to Proton population remains too small to mark this task complete or cite a general rate.
