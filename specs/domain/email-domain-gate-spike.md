...
- [ ] A2c. **Received-mail archive coverage survey:** run `survey-mail-archive.mjs` over a recent, local MBOX export from an opt-in mailbox. Stream headers only; exclude list/bulk/automated and sent-folder messages before DNS; cryptographically verify remaining header signatures under A2b; collapse repeated correspondents to one best observed verdict per From domain; commit counts only. Record the date window, filter counts, DNS-query disclosure, removed-selector/archive-age false-negative risk, export-fidelity failures, and correspondent/sector bias. This estimates compatibility for the mailbox's actual human-business correspondence; it is not a population census or an organization-wide claim.
