...
2. **Signed-field coverage:** are From and Subject demonstrably inside `h=` across the matrix (incl. multiple-signature messages)? Is exactly-one-author-address parsing from the signed canonicalized header robust, including duplicate From headers?
...
- [ ] A2. For each: record `d=`, `s=`, `h=` list, canonicalization (`c=`), key alg/size, whether From/Subject are signed, then cryptographically verify the signature and body hash against the published DNS key. Structural inspection is triage only and cannot support a provider verdict. Produce the alignment-and-verification pass-rate table → feeds go/no-go Q1/Q2.
...
| T13a | Two From headers with otherwise valid signed content | FAIL; a regex silently selecting either occurrence is unacceptable |
