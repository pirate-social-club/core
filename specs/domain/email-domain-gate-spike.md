...
6. **Circuit obligations.** Valid DKIM signature against the pinned key; `h=` provably covers Subject and oversigns From (lists `from` at least twice) until the circuit itself proves From cardinality or binds the last From occurrence; exactly one author address parsed from the same signed From occurrence the circuit authenticates; subject matches the approved nonce template with nonce public; `d=` exactly equals the From domain (no relaxed/organizational alignment at launch — that requires a maintained PSL). Public outputs: From-domain, nonce, hashed From for best-effort dedup. To is not extracted or exposed. Header-only circuit; body ignored.
...
2. **Signed-field coverage:** are Subject and an oversigned From (`from` at least twice in `h=`) demonstrably covered across the matrix (incl. multiple-signature messages)? Does the circuit extract the same From occurrence DKIM authenticates, and is duplicate-From parsing robust?
...
- [ ] A2. For each: record `d=`, `s=`, full `h=` list including the number/position of `from` occurrences, canonicalization (`c=`), key alg/size, and Subject coverage, then cryptographically verify the signature and body hash against the published DNS key. Until circuit cardinality/last-occurrence binding is proven, fewer than two `from` entries is gate-ineligible even if the signature otherwise verifies. Structural inspection is triage only. Produce the alignment/oversigning/verification pass-rate table → feeds go/no-go Q1/Q2.
...
### Phase B — blueprint
- [ ] B1. Author registry blueprint (docs: zk-email-sdk/creating-a-new-pattern): DKIM verify, `h=` coverage assertions, exactly-one-From behavior tested adversarially, subject nonce public, From-domain public, unsalted hashed From output for best-effort dedup, header-only. Do not extract To.
- [x] B2 desk check. `@zk-email/sdk@2.0.11` can expose/hash regex extractions for the subject nonce, From domain, and best-effort From digest. However, the ordinary blueprint fixes one `senderDomain`, and the schema cannot compose an extracted mailbox with an application salt or express From cardinality directly. Q4's badge-only decision makes the hosted unsalted `isHashed` output acceptable if it is immediately HMACed and never logged, returned, or placed in a URL. Any escaped raw digest remains universally enumerable and cross-application linkable. Generated-project validation must document exactly what `isHashed` commits to, feed a duplicate-From adversarial message, and test the lower-level dynamic-domain wrapper. Canonical mailbox extraction is a dedup-quality property rather than a launch security blocker. See `spikes/email-domain-gate/b2-blueprint-dsl.md`.
...
| T13a | Two From headers with otherwise valid signed content | FAIL; prove the circuit reads the DKIM-authenticated occurrence. With first-match extraction, require `from` at least twice in `h=` so prepending another From invalidates DKIM |
