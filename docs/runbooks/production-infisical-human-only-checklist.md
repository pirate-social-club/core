...
```bash
cd /run/media/x42/codedrive/Code/pirate-workspace/core
...
```bash
cd /run/media/x42/codedrive/Code/pirate-workspace/core/ops/prod
...
export CREDENTIAL_WRAP_KEY="$(openssl rand -hex 32)"
```
...
rtk infisical secrets set \
  ...
  STORY_ENTITLEMENT_CLASS_CONFIGURER_PRIVATE_KEY="$STORY_ENTITLEMENT_CLASS_CONFIGURER_PRIVATE_KEY" \
