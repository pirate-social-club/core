...
- [lit-action-port-plan.md](/home/t42/Documents/pirate-v2/docs/lit-action-port-plan.md)

## Model

Every Lit-backed signer family follows this structure:

1. **Signer family** — the on-chain identity and authority. Defined in `docs/signer-families.md`.
...
|---|---|
| Signer address, on-chain authority, grants | `docs/signer-families.md` |
...
| Funded wallet balances and refill policy | `docs/funds-ledger.md` |
...
This policy does not prohibit explicit direct-key families. If a family is defined as `direct-key` in `docs/signer-families.md`, that is its primary signer kind, not a fallback from PKP.
...
Run the local checker after editing `config/lit-families.json`, `docs/signer-families.md`, or `docs/funds-ledger.md`:
...
- config-declared funded-address placeholders in `docs/funds-ledger.md`
...
6. Update Infisical, `config/lit-families.json`, and `docs/signer-families.md`.
...
- Each PKP address has the required on-chain grants per `docs/signer-families.md`.
- Each funded PKP address has a balance above the floor defined in `docs/funds-ledger.md`.
...
`story-scrobble-operator` is intentionally out of scope for the Lit control plane in v0. Pirate-v2 batch scrobble anchoring uses a dedicated direct-key publisher on `ScrobbleV1`, defined in `docs/signer-families.md` and funded via `docs/funds-ledger.md`. It is not represented in `config/lit-families.json` until a future PKP migration is real.
