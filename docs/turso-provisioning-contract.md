...
- community Turso group name: `club-<community_id>`
...
3. Create Turso group `club-<community_id>`.
4. Create primary database `main` in that group.
5. Mint a database-scoped runtime token for `main`.
...
The first human-run interface should look roughly like:

```bash
rtk bun scripts/turso-control-plane.ts provision-community --club-id gld_01... --creator-user-id usr_01... --group-location iad
rtk bun scripts/turso-control-plane.ts rotate-community-token --club-id gld_01...
rtk bun scripts/turso-control-plane.ts transfer-community --club-id gld_01... --target-org community-org --post-transfer-mode federated
rtk bun scripts/turso-control-plane.ts doctor
```

The exact runtime can change later. The operation names and durable side effects should not.
