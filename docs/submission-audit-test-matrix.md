...
- [song publish state machine](/home/t42/Documents/pirate-v2/docs/song-publish-v0-state-machine.md)
...
| `SCN-INF-002` | Inspect the bootstrapped Infinity community in control-plane state | `status = active`, `provisioning_state = active`, `registry_publication_state = not_started` |
...
| `SCN-INF-004` | Confirm the Infinity local-stub scenario is not misreported as Tableland-published | community remains non-canonical for public discovery until a distinct registry-publication flow sets `registry_publication_state = published` |
