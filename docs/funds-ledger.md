...
- signer-family canonical addresses: `docs/signer-families.md`
- PKP control-plane addresses: `config/lit-families.json`

When those files disagree, this ledger is not authoritative on which address is correct. It is authoritative on which addresses hold value and what to do about it. Direct-key families such as `story-scrobble-operator` resolve from `docs/signer-families.md` only.
...
- **contract-owner**: the deployer address that holds ownership roles on deployed contracts. Defined in `docs/signer-families.md` per chain. Currently a direct key, intended migration target is a multisig.
