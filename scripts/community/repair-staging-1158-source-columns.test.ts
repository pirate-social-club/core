...
describe("1158 additive source-column repair", () => {
  ...
  test("reproduces canonical declarations and converges", () => {
    const probe = {
      columns: [{ table: "posts", name: "lyrics_language" }],
      ledgers: checksums,
    }
    ...
    expect(planRepair({ columns: REPAIRS.flatMap(({ columns }) => columns.map(([table, name]) => ({ table, name }))), ledgers: checksums }, checksums)).toEqual({ kind: "converged" })
  })
})
